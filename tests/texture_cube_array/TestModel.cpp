...
TestModel::TestModel(GraphicsCore& graphics, ShaderResourceDescHeap* shader_buffer_heap, CommandList* command_list)
{
  ...
  try
  {
    m_texture = TextureCubeArray::CreateD3D12(graphics, *shader_buffer_heap, TEXTURE_WIDTH, TEXTURE_HEIGHT, NUM_CUBES, (GraphicsDataFormat)TEXTURE_FORMAT);
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  upload_texture.reserve(TEXTURES_PER_CUBE);
  try
  {
    for (UINT c = 0; c < NUM_CUBES; c++)
    {
      ...
      {
        upload_texture.push_back(TextureUploadBuffer::CreateD3D12(graphics, *m_texture));
      }
    }
  }
  catch (const FrameworkException& err)
  ...
  try
  {
    for (UINT c = 0; c < NUM_CUBES; c++)
    {
      for (UINT16 i = 0; i < TEXTURES_PER_CUBE; i++)
      {
        ...
        upload_texture[c * TEXTURES_PER_CUBE + i]->PrepUpload(graphics, *command_list, *m_texture, c, i, tex_bytes);
      }
    }
  }
  catch (const FrameworkException& err)
...
const TextureCubeArray* TestModel::GetTexture() const
{
  return m_texture;
}

void TestModel::CreateTexture(UINT16 cube, UINT16 side, vector<UINT8>& bytes)
{
