 ...
 m_camera(NULL)
{
  ...
  try
  {
    vertex_shader = Shader::LoadD3D12("fps_monitor_test_vs.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    pixel_shader = Shader::LoadD3D12("fps_monitor_test_ps.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
void TestGraphicsPipeline::SetFPSMonitor(const FPSMonitor* monitor)
{
  m_fps_monitor = monitor;
}
