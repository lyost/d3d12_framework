...
TestModel::TestModel(GraphicsCore& graphics, ShaderResourceDescHeap* shader_buffer_heap, CommandList* command_list)
{
  ...
  try
  {
    m_verts = VertexBuffer_PositionTextureUV::CreateD3D12(graphics, ARRAYSIZE(vertices), vertices);
  }
  ...
  try
  {
    m_texture = Texture2D::CreateD3D12(graphics, *shader_buffer_heap, RTV_TEX_WIDTH, RTV_TEX_HEIGHT, (GraphicsDataFormat)RTV_TEX_FORMAT);
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
const VertexBuffer_PositionTextureUV* TestModel::GetVertexBuffer() const
{
  return m_verts;
}
...
Texture2D* TestModel::GetTexture() const
{
  return m_texture;
}
