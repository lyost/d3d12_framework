...
TestGraphicsPipelinePosColor::TestGraphicsPipelinePosColor(GraphicsCore& graphics)
:m_model(NULL),
 m_camera(NULL)
{
  try
  ...
  try
  {
    vertex_shader = Shader::LoadD3D12("vertex_buffer_upload_poscolor_vs.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    pixel_shader = Shader::LoadD3D12("vertex_buffer_upload_poscolor_ps.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
TestGraphicsPipelinePosColor::~TestGraphicsPipelinePosColor()
{
...
ShaderResourceDescHeap* TestGraphicsPipelinePosColor::GetShaderResourceDescHeap() const
{
  return m_shader_buffer_heap;
}

void TestGraphicsPipelinePosColor::SetModel(const TestModelPosColor* model)
{
  m_model = model;
...
void TestGraphicsPipelinePosColor::SetCamera(const Camera* cam)
{
  m_camera = cam;
}

void TestGraphicsPipelinePosColor::Draw(GraphicsCore& graphics, CommandList& command_list)
{
  try
  {
