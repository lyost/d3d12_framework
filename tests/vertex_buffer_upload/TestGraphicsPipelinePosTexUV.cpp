...
TestGraphicsPipelinePosTexUV::TestGraphicsPipelinePosTexUV(GraphicsCore& graphics)
:m_model(NULL),
 m_camera(NULL)
{
  try
  ...
  try
  {
    vertex_shader = Shader::LoadD3D12("vertex_buffer_upload_postexuv_vs.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    pixel_shader = Shader::LoadD3D12("vertex_buffer_upload_postexuv_ps.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
TestGraphicsPipelinePosTexUV::~TestGraphicsPipelinePosTexUV()
{
...
ShaderResourceDescHeap* TestGraphicsPipelinePosTexUV::GetShaderResourceDescHeap() const
{
  return m_shader_buffer_heap;
}

void TestGraphicsPipelinePosTexUV::SetModel(const TestModelPosTexUV* model)
{
  m_model = model;
...
void TestGraphicsPipelinePosTexUV::SetCamera(const Camera* cam)
{
  m_camera = cam;
}

void TestGraphicsPipelinePosTexUV::Draw(GraphicsCore& graphics, CommandList& command_list)
{
  try
  {
