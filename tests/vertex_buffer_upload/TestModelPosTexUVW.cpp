...
TestModelPosTexUVW::TestModelPosTexUVW(GraphicsCore& graphics, ShaderResourceDescHeap* shader_buffer_heap, CommandList& command_list)
{
  ...
  m_vert_data[0] = { XMFLOAT3(3.0f,-3.0f, 1.0f), XMFLOAT3(0.0f, 0.0f, 0.0f) };
  m_vert_data[1] = { XMFLOAT3(4.0f,-3.0f, 1.0f), XMFLOAT3(1.0f, 0.0f, 0.0f) };
  m_vert_data[2] = { XMFLOAT3(4.0f,-2.0f, 1.0f), XMFLOAT3(1.0f, 1.0f, 1.0f) };
  m_vert_data[3] = { XMFLOAT3(3.0f,-2.0f, 1.0f), XMFLOAT3(0.0f, 1.0f, 1.0f) };
  try
  {
    m_verts = VertexBuffer_PositionTextureUVW::CreateD3D12(graphics, ARRAYSIZE(m_vert_data), m_vert_data);
  }
  ...
  try
  {
    m_gpu_verts = VertexBufferGPU_PositionTextureUVW::CreateD3D12(graphics, m_verts->GetNumVertices());
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  CreateTexture(width, height, depth, format, tex_bytes);

  // create the texture
  try
  {
    m_texture = Texture3D::CreateD3D12(graphics, *shader_buffer_heap, width, height, depth, format);
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
TestModelPosTexUVW::~TestModelPosTexUVW()
{
  delete m_texture;
  delete m_indices;
  delete m_verts;
...
void TestModelPosTexUVW::UpdateVertexBuffer(GraphicsCore& graphics, bool initial, CommandList& command_list)
{
  if (initial)
  {
    m_vert_data[0] = { XMFLOAT3(3.0f,-3.0f, 1.0f), XMFLOAT3(0.0f, 0.0f, 0.0f) };
  ...
  {
    m_vert_data[0] = { XMFLOAT3(3.0f,-3.0f, 1.0f), XMFLOAT3(0.25f, 0.25f, 0.25f) };
    m_vert_data[1] = { XMFLOAT3(4.0f,-3.0f, 1.0f), XMFLOAT3(0.75f, 0.25f, 0.25f) };
    m_vert_data[2] = { XMFLOAT3(4.0f,-2.0f, 1.0f), XMFLOAT3(0.75f, 0.75f, 0.75f) };
    m_vert_data[3] = { XMFLOAT3(3.0f,-2.0f, 1.0f), XMFLOAT3(0.25f, 0.75f, 0.75f) };
  }
  try
  {
...
const VertexBufferGPU_PositionTextureUVW* TestModelPosTexUVW::GetVertexBuffer() const
{
  return m_gpu_verts;
}

const IndexBufferGPU16* TestModelPosTexUVW::GetIndexBuffer() const
{
  return m_gpu_indices;
}

const Texture3D* TestModelPosTexUVW::GetTexture() const
{
  return m_texture;
}

void TestModelPosTexUVW::CreateTexture(UINT& width, UINT& height, UINT16& depth, GraphicsDataFormat& format, std::vector<UINT8>& bytes)
{
  width  = 64;
  height = 64;
  depth  = 3;
