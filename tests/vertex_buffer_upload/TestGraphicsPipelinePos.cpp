...
TestGraphicsPipelinePos::TestGraphicsPipelinePos(GraphicsCore& graphics)
:m_model(NULL),
 m_camera(NULL)
{
  try
  ...
  try
  {
    vertex_shader = Shader::LoadD3D12("vertex_buffer_upload_pos_vs.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    pixel_shader = Shader::LoadD3D12("vertex_buffer_upload_pos_ps.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
TestGraphicsPipelinePos::~TestGraphicsPipelinePos()
{
...
ShaderResourceDescHeap* TestGraphicsPipelinePos::GetShaderResourceDescHeap() const
{
  return m_shader_buffer_heap;
}

void TestGraphicsPipelinePos::SetModel(const TestModelPos* model)
{
  m_model = model;
...
void TestGraphicsPipelinePos::SetCamera(const Camera* cam)
{
  m_camera = cam;
}

void TestGraphicsPipelinePos::Draw(GraphicsCore& graphics, CommandList& command_list)
{
  try
  {
