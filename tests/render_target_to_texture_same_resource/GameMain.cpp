...
void GameMain::LoadContent()
{
  ...
  try
  {
    m_vertex_shader = Shader::LoadD3D12("render_target_to_texture_same_resource_vs.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    m_pixel_shader = Shader::LoadD3D12("render_target_to_texture_same_resource_ps.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
void GameMain::CreateRTV()
{
  ...
  try
  {
    m_texture = Texture2DRenderTarget::CreateD3D12(graphics, *m_shader_buffer_heap, RTV_TEX_WIDTH, RTV_TEX_HEIGHT, (GraphicsDataFormat)RTV_TEX_FORMAT);
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    m_rtv_texture = RenderTarget::CreateD3D12FromTexture(graphics, *m_texture);
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
...
void GameMain::DrawRTV()
{
  try
  {
    ...
    m_command_list->TextureToRenderTarget(*m_texture);
