 ...
 m_vert_array(main_pipeline.m_vert_array)
{
  ...
  try
  {
    ...
    m_rtv_pipeline = Pipeline::CreateD3D12(graphics, *main_pipeline.m_input_layout, TOPOLOGY_TRIANGLE, *main_pipeline.m_vertex_shader, NULL, *m_rtv_pixel_shader, &ds_config, *rtv_config, *m_root_sig);
    delete rtv_config;
  }
  catch (const FrameworkException& err)
  {
