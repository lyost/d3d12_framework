...
void GameMain::LoadContent()
{
  ...
  m_vertex_shader = Shader::LoadD3D12("geometry_shader_viewports_vs.cso");
  if (m_vertex_shader == NULL)
  {
    exit(1);
  }
  m_geometry_shader = Shader::LoadD3D12("geometry_shader_viewports_gs.cso");
  if (m_geometry_shader == NULL)
  {
    exit(1);
  }
  m_pixel_shader = Shader::LoadD3D12("geometry_shader_viewports_ps.cso");
  ...
  m_pipeline = Pipeline::CreateD3D12(graphics, *m_input_layout, TOPOLOGY_TRIANGLE, *m_vertex_shader, *m_geometry_shader, *m_pixel_shader, DEPTH_FUNC_LESS_EQUAL, *rtv_config, *m_root_sig);
  if (m_pipeline == NULL)
  {
    exit(1);
  }
  ...
  UINT constant_buffer_size = ConstantBuffer::GetAlignedSize(graphics, sizeof(XMMATRIX) * NUM_VIEWPORTS);
...
void GameMain::Draw(UINT ms)
{
  ...
  for (int i = 0; i < NUM_VIEWPORTS; i++)
  {
    ...
    m_camera[i]->GetView(tmp);
    wvp *= tmp;
    m_camera[i]->GetProjection(tmp);
    wvp *= tmp;
