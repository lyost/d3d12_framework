...
void GameMain::LoadContent()
{
  ...
  try
  {
    m_vertex_shader = Shader::LoadD3D12("texture_cube_vs.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    m_pixel_shader = Shader::LoadD3D12("texture_cube_ps.cso");
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    m_texture = TextureCube::CreateD3D12(graphics, *m_shader_buffer_heap, TEXTURE_WIDTH, TEXTURE_HEIGHT, (GraphicsDataFormat)TEXTURE_FORMAT);
  }
  catch (const FrameworkException& err)
  {
    ostringstream out;
  ...
  try
  {
    for (UINT16 i = 0; i < TEXTURES_PER_CUBE; i++)
    {
      ...
      m_upload_texture[i]->PrepUpload(graphics, *m_command_list, *m_texture, i, tex_bytes);
    }
  }
  catch (const FrameworkException& err)
  {
...
void GameMain::CreateTexture(UINT16 side, vector<UINT8>& bytes)
{
