...
void GameMain::Update(UINT step_ms, UINT actual_ms)
{
  ...
  try
  {
    ...
    else if (spacebar_down)
    {
      ...
      TextureModes mode = m_pipeline->GetMode();
      mode = (TextureModes)((mode + 1) % TEXTURE_MODE_MAX);
      m_pipeline->SetMode(mode);
    }
    else if (keyboard.IsKeyDown(VK_NUMPAD1, false))
    {
      m_pipeline->SetMipmapLevel(0);
    }
    else if (keyboard.IsKeyDown(VK_NUMPAD2, false))
    {
      m_pipeline->SetMipmapLevel(1);
    }
