#ifndef D3D12_RENDER_TARGET_DESC_HEAP_H
#define D3D12_RENDER_TARGET_DESC_HEAP_H

#include <d3d12.h>

class D3D12_RenderTargetDescHeap
{
  public:
    /// <summary>
    /// Creates a D3D12 descriptor heap for render targets
    ///</summary>
    /// <param name="device">
    /// D3D12 device
    /// </param>
    ...
    /// </exception>
    static D3D12_RenderTargetDescHeap* Create(ID3D12Device* device, UINT num_descriptors);

    ~D3D12_RenderTargetDescHeap();
  ...
  private:
    // disabled
    D3D12_RenderTargetDescHeap();
    D3D12_RenderTargetDescHeap(const D3D12_RenderTargetDescHeap& cpy);
    D3D12_RenderTargetDescHeap& operator=(const D3D12_RenderTargetDescHeap& cpy);

    D3D12_RenderTargetDescHeap(UINT num_descriptors, const D3D12_CPU_DESCRIPTOR_HANDLE& cpu_start, const D3D12_GPU_DESCRIPTOR_HANDLE& gpu_start, UINT descriptor_size, ID3D12DescriptorHeap* heap);
...
#endif /* D3D12_RENDER_TARGET_DESC_HEAP_H */
