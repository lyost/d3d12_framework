...
{
  return D3D12_Pipeline::Create(graphics_core, input_layout, topology, vertex_shader, pixel_shader, rtv_config, root_sig);
}
...
Pipeline* Pipeline::CreateD3D12(const GraphicsCore& graphics_core, const InputLayout& input_layout, Topology topology, const Shader& vertex_shader, const Shader& pixel_shader, DepthFuncs depth_func,
  const RenderTargetViewConfig& rtv_config, const RootSignature& root_sig)
{
  return D3D12_Pipeline::Create(graphics_core, input_layout, topology, vertex_shader, pixel_shader, depth_func, rtv_config, root_sig);
}
...
{
  return D3D12_Pipeline::Create(graphics_core, input_layout, topology, vertex_shader, geometry_shader, pixel_shader, depth_func, rtv_config, root_sig);
}
